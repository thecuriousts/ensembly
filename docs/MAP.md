...
| **Pulse pack sync** | `cargo run -p peram-kernel -- pulse-pack export\|import\|status` | Portable `peram-pulse-pack-v1` JSON: episodic memory traces + optional audit archive slice. **Import merges CRDT memory only** — does not dual-write `peram-ops.sqlite`. Archive rows land in `data/local/pulse-archive.jsonl`. Host roles: Grok Bot = kernel SoT; laptop = client. Recipe: [PLAYBOOK.md](PLAYBOOK.md#12-pulse--memory-sync-bot--laptop) |
