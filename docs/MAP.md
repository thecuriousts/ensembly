...
|------------|-------|--------------|
| **Kernel CLI** | `cargo run -p peram-kernel -- …` · `npm run peram -- …` | `rank_now` FocusPlan, bill_pay dry-run, T1 SQLite, sealed backup (`backup` / `restore-dry-run` / `restore-apply`), unsealed ops bundle (`ops-bundle`); Operator CLI lives in `bin/swarm.js` + `src/*` |
| **Runtime S+G+CP+P (Issue #1)** | `cargo run -p peram-kernel -- runtime load\|status\|tick\|approve\|deny\|claim\|complete\|dive` · fixture `fixtures/issue-1-runtime.json` | Life-state **S** + dependency graph **G** + critical path / PERT+Monte Carlo **P**; typed message bus; Human-Out-Of-The-Loop agents claim digital thrash; authorization gate + physical beacon wait only for permission. Persist to `data/local/peram-ops.sqlite` (gitignored). **Approve/deny id = action id** (e.g. `pay-rent`), not `auth-pay-rent` from `pendingAuth`. `runtime * --json` emits JSON then a trailing `RUNTIME_OK …` line. |
| **UncertaintyDive** | `cargo run -p peram-kernel -- runtime dive [--json] [--probe-budget N]` | Inspect-only Prior→Probe→Simulate→Score→ActOrAsk over G+CP; Auth = black hole (HITL only); trauma guards (no auto Auth/Physical, Claim-via-CP, budget). Quest: [thinking/uncertainty-space-quest.md](thinking/uncertainty-space-quest.md) · [DECISIONS.md](DECISIONS.md#uncertaintydive--process-under-epistemic-emptiness-2026-07-30) · spacemap SN-9 |
...
| **Pulse pack sync** | `cargo run -p peram-kernel -- pulse-pack export\|import\|status` | Portable `peram-pulse-pack-v1` JSON — **pulse layer only** (`memory_traces` + `archive_events`). Separate from T1 `BackupPack` / `ops-bundle`. Import merges CRDT memory only; does not dual-write `peram-ops.sqlite`. Host roles: Grok Bot = kernel SoT; laptop = client. Recipe: [PLAYBOOK.md](PLAYBOOK.md#12-pulse--memory-sync-bot--laptop) |
...
| **`crates/peram-agents`** | Protocol satellite: `InferenceProvider`, `peram-mcp` (read-only MCP for Grok/Cursor), Grok CLI helpers (`grok agent stdio`, `grok mcp add`) | Control SoT, gates — kernel stays Eagle |
