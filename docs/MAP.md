...
| **Episodic memory (peram-memory)** | `cargo run -p peram-kernel -- runtime reflect [--json]` · `--memory <path>` / `--no-memory` | Runtime records applied bus messages, ticks, loads into a durable CRDT trajectory at `data/local/peram-memory.json` (gitignored). `reflect` = coherence + skill synthesis + goal proposals (skips loudly < 5 entries). **Aux learning layer — kernel stays control SoT; memory never decides.** Decision: [DECISIONS.md](DECISIONS.md#episodic-memory-layer--peram-memory-fused-from-intelliarch-2026-07-29) |
