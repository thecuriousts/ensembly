...
| **UncertaintyDive** | `cargo run -p peram-kernel -- runtime dive [--json] [--probe-budget N]` | Inspect-only Prior→Probe→Simulate→Score→ActOrAsk over G+CP; Auth = black hole (HITL only); trauma guards (no auto Auth/Physical, Claim-via-CP, budget). Quest: [thinking/uncertainty-ocean-quest.md](thinking/uncertainty-ocean-quest.md) · [DECISIONS.md](DECISIONS.md#uncertaintydive--process-under-epistemic-emptiness-2026-07-30) |
